...
Symbol prefix in examples: `<pfx>` in prose, `ba_` in code. Substitute the
...
|---|---|---|---|---|
| basalt | WinAPI-only foundation lib (crt, string, vector, json, socket, http) | C17 + optional C++ layer | `ba_` | no CRT dependency by design |
...
- Choose a symbol prefix at repo creation: 2–4 lowercase letters + `_`,
  unique across the workspace, never changed afterwards. Add a row here.
...
| Header guard | `<LIB>_<FILE>_H` | `BA_HTTP_H` |
...
|---|---|---|
| C public functions | `<pfx>_<module>_<verb>_<object>`, snake_case | `ba_http_request_send()` |
| C internal functions | snake_case, **no prefix**, always `static` | `parse_status_line()` |
| C structs / enums (public) | `<pfx>_<name>` lowercase typedef | `ba_http_response` |
...
| C enum values / macros / guards | `<LIB>_` + SCREAMING_SNAKE | `BA_HTTP_GET`, `BA_ERR_MEM` |
| Namespaces | lowercase, one word, = project name | `namespace basalt` |
...
| Lifecycle pairs | `create` / `destroy` only — never `init/free` mixed in | `ba_socket_create()` |
...
- The highest-value pair in the table is **`c` vs `cb`**: confusing element
  ...
  at every call site.
- An enum involves **three** names: the type (`ba_http_method` — §4 type
  rule), its values (`BA_HTTP_GET` — §4 constant rule), and variables of
  that type (`e_method` — this table). §15's status enum follows all three.
- Pointers to C handles take `p` + handle name: `p_sock` for
  `ba_socket*`. Inside C++ classes, members are the object — no `p` needed.
...
the namespaced C++ class and the prefixed C API.
...
typedef struct ba_socket ba_socket;              /* opaque handle */

ba_socket* ba_socket_create(void);
void       ba_socket_destroy(ba_socket* p_sock);
ba_status  ba_socket_connect(ba_socket* p_sock,
                             const char* sz_host, uint16_t u16_port);
ba_status  ba_socket_send(ba_socket* p_sock,
...
struct ba_socket {                               /* opaque half — this file only */
    SOCKET     h_sock;
...
static ba_status net_init_once(void)             /* WSAStartup exactly once */
{
...
ba_socket* ba_socket_create(void)
{
    ...
    ba_socket* p_sock  = (h_heap != NULL)
        ? (ba_socket*)HeapAlloc(h_heap, HEAP_ZERO_MEMORY, sizeof(*p_sock))
        : NULL;
                            ...
                            const char* sz_host, uint16_t u16_port)
{
    ...
    ba_status   e_status = BA_OK;                /* enum variable: e_ prefix */
...
cleanup:
    if (e_status != BA_OK && p_sock->h_sock != INVALID_SOCKET) {
...
ba_status ba_socket_send(ba_socket* p_sock, const void* p_buf, size_t cb_len)
{
...
```cpp
/* SPDX-License-Identifier: MIT */
#ifndef BA_SOCKET_CPP_HPP
#define BA_SOCKET_CPP_HPP
...
public:
    ...
    {
        close();
        p_sock_ = ba_socket_create();
        if (p_sock_ == nullptr)
            return false;
        return ba_socket_connect(p_sock_, sz_host, u16_port) == BA_OK;
    }

    bool send(const void* p_buf, size_t cb_len)
    {
        return p_sock_ != nullptr
            && ba_socket_send(p_sock_, p_buf, cb_len) == BA_OK;
    }

    void close()
    {
        ba_socket_destroy(p_sock_);
        p_sock_ = nullptr;
    }

private:
    ba_socket* p_sock_ = nullptr;                /* type prefix + member marker */
};

} // namespace basalt
#endif /* BA_SOCKET_CPP_HPP */
```
...
int main()
{
    /* C++ door — namespace + RAII; application code never types ba_ */
    ...
    ba_socket* p_sock = ba_socket_create();
    if (ba_socket_connect(p_sock, "127.0.0.1", 9000) == BA_OK)
        ba_socket_send(p_sock, "ping", 4);
    ba_socket_destroy(p_sock);
    return 0;
}
```
